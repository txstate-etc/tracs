...
//			appendJavaScript("Window.open(url, '_blank', strWindowFeature)");
//			getSession().success(MessageFormat.format(getString("message.gradesubmit.success"), gradeSubmission + " HIII  " + data));
//			setResponsePage(getPage().getPageClass());
			
			//following two lines are just to check for look and feel of errors
//			error(new StringResourceModel("message.gradesubmit.error2", null, new Object[] {gradeSubmission}).getObject());
//			form.add(new GbFeedbackPanel("addGradeSubmitFeedback"));
		}
		else {
		...
		final ExternalLink link = new ExternalLink("gradeSubmitLink", "https://yahoo.com") {
			@Override
		...
		final GbAjaxButton submit = new GbAjaxButton("submit") {
			...
			public String getOnClickScript(){
				...
				return "Window.open(url, '_blank', strWindowFeature)";
			}
			@Override
