...
GradebookSpreadsheet.prototype.navigate = function(event, fromCell, direction, enableEditMode) {
  ...
  if (direction == "left") {
    ...
    if ($cell.index() < $row.children().last().index() && $cell.index() > 2) {
...
GradebookSpreadsheet.prototype.setupCloneTables = function() {
    ...
    leftblock.on('keydown', 'tbody', false);
    leftblock.on('keydown', 'thead', function(event) {
...
GradebookSpreadsheet.prototype.refreshCourseGradeForStudent = function(studentUuid) {
  ...
  var $fixedColumnStudentNameCell = this.$cloneLeft.find(".gb-student-cell[data-studentuuid='"+studentUuid+"']");
