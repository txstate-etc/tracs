...
public class ForumsEntityProviderImpl extends AbstractEntityProvider implements Outputable, AutoRegisterEntityProvider, ActionsExecutable, Describeable {
	...
	/**
	 * Handles the path /direct/forums/user
	 *
	 * @param view   unused
	 * @param params unused
	 * @return A json friendly HashMap containing with siteId keys and an array of accessible forum messages as values.
	 */
	@EntityCustomAction(action = "user", viewKey = EntityView.VIEW_LIST)
	public Object handleUser(EntityView view, Map<String, Object> params) {
		...
		if (userId == null)
			throw new EntityException("You must be logged in to retrieve your fora.", "", HttpServletResponse.SC_UNAUTHORIZED);
		...
		for (Site site : userSites) {
			...
			if (userCannotAccessSiteAndTool(siteId)) continue;
			List forums = forumManager.getDiscussionForumsWithTopicsMembershipNoAttachments(siteId);
			for (Object forum : forums) {
				Long forumId = ((DiscussionForum) forum).getId();
				DiscussionForum discussionForum = forumManager.getForumByIdWithTopicsAttachmentsAndMessages(forumId);
				List topics = discussionForum.getTopics();
				for (Object topic : topics) {
					DiscussionTopic discussionTopic = (DiscussionTopic) topic;
					if (!uiPermissionsManager.isRead(discussionTopic, discussionForum)) continue;
					if (discussionTopic == null) continue;
					List messages = discussionTopic.getMessages();
					for (Object setMessage : messages) {
						...
						msg.put("created", message.getCreated().toString());
						msg.put("lastModified", message.getModified().toString());
						msg.put("siteTitle", site.getTitle());
						msg.put("forumTitle", discussionForum.getTitle());
						msg.put("topicTitle", discussionTopic.getTitle());
